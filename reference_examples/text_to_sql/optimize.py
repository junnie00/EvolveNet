...
(load_harness, PKG_DIR, acc, ...), so they are dependencies — do not delete them.
    ...
    cd meta-harness-ref && ANTHROPIC_BASE_URL=... ANTHROPIC_AUTH_TOKEN=... TTHO_PROPOSER_MODEL=deepseek-v4-flash \
      YINLI_API_KEY=... YINLI_KEY_EXAMINER=... python -m reference_examples.text_to_sql.optimize \
...
def isolated_solve(run_dir, harness_name, db_id, question, timeout, tag):
    ...
    # Fix the hash seed. Every solve is a fresh process, so by default Python randomises string hashing
    # per process and any harness that collects column or table names in a `set` iterates them in a
    # different order each time. Proposers write such code routinely. The order reaches the prompt via
    # the probe results, which changes the solver-cache key, which forces a fresh sample — one harness
    # was observed probing the same four columns in two different orders and diverging into completely
    # different SQL from there. Pinning the seed makes set iteration reproducible so the cache can hit.
...
def main():
    ...
    ap.add_argument("--public-val", default=None,
                    help="cross-set slice broadcast to every client and used to SELECT among candidates "
                         "instead of the client's own shard (cuts shard-overfitting at the source). "
                         "Private shards never leave the client.")
    ap.add_argument("--specialist", action="store_true",
                    help="federated: tell each client's proposer to become the EXPERT of its own slice "
                         "(mine slice-specific schema/failure knowledge) instead of writing a GENERIC "
                         "harness. Without it, clients converge on the same generic mechanisms and there "
                         "is nothing to aggregate (measured 83-94%% pairwise agreement). Off by default "
                         "so centralized runs are unchanged.")
    ap.add_argument("--role-offset", type=int, default=0,
                    help="shift the proposer search role by this much (federated: pass the client index so "
                         "roles spread across clients rather than within one)")
    ...
    ap.add_argument("--supervised", action="store_true",
                    help="use the shard's gold labels INSIDE the loop: traces state whether each answer was "
                         "correct, and selection scores candidates by measured accuracy instead of a judge's "
                         "label-free guess. Valid only for train-then-test (labelled training shards); the "
                         "original transductive protocol must leave this off.")
    ...
    if args.fresh:
        for f in AGENTS_DIR.glob("*.py"):
            if f.name not in ("__init__.py", "bare.py"):
                f.unlink()
    ...
    def write_trace(trace_dir, name, j, db_id, question, schema, outcome, verdict=None):
        """Write ONE harness's FULL trace on ONE batch question (question + Hint + schema + every step + back-translation).

        `verdict` is the gold verdict for this question, included ONLY in supervised mode. The original
        loop is label-free because it adapts on an unlabeled test stream; this fork trains on labelled
        shards first and then tests, so withholding the label here buys nothing and starves the proposer
        of the one signal that says WHICH questions to attack.
        """
        sql = outcome.get("sql", "")
        ...
        if verdict is not None:
            L.append(f"## GRADED OUTCOME (ground truth for this training question)\n"
                     f"**This harness answered this question {'CORRECTLY' if verdict else 'INCORRECTLY'}.**\n"
                     f"{'Preserve whatever produced this.' if verdict else 'This is a question to fix. The final result below does NOT match the expected answer, even if the SQL ran without error.'}\n")
        L += [f"## QUESTION\n{qtext.strip()}\n",
    ...
    def observe(name, batch, trace_dir, golds=None):
        ...
        `golds` is passed only in supervised mode, so each trace states whether the answer was right."""
        ...
        def one(jdbq):
            ...
            if golds is not None:
                verdict = bool(bridge.is_correct(outcome["result"], golds[j]))
            write_trace(trace_dir, name, j, db_id, q.question, schema, outcome, verdict)
    ...
    pub_items, pub_golds = [], []
    if args.supervised and args.public_val:
        spec = json.load(open(args.public_val))["cross"]
        pub_items = [(d, bridge.eval_questions(d)[i]) for d, i in spec]
        pub_golds = [bridge.gold_result(getdb(d)[0], q.gold_sql) for d, q in pub_items]
        print(f"[client] candidate selection uses the PUBLIC set ({len(pub_items)} questions), "
              f"not the private shard", flush=True)
    ...
    for bi in range(args.start_batch, len(batches)):
        ...
        gold_for_loop = batch_golds[bi] if args.supervised else None   # None keeps the loop label-free
        entering_H = H                                             # what was handed to this batch (the broadcast global)
        ...
        for rnd in range(args.max_rounds):
            ...
            for c in active:
                if c not in traced and _loadable(c, db0):
                    cand_results[c] = observe(c, batch, trace_dir, gold_for_loop)
                    traced.add(c)
        ...
        for c in final_candidates:
            if c not in traced and _loadable(c, db0):
                ...
                traced.add(c)
        if args.supervised:
            # Score every candidate on this shard's labels and keep the best. The harness that ENTERED the
            # batch is always in the running, so a client can never hand back something measurably worse
            # than what it was given. Ties go to the incumbent (prefer not to churn on no evidence).
            pool = list(dict.fromkeys(final_candidates + [entering_H]))
            for c in pool:
                if c not in traced and _loadable(c, db0):
                    cand_results[c] = observe(c, batch, trace_dir, gold_for_loop)
                    traced.add(c)
            if pub_items:
                # Select on the PUBLIC set, not the shard the proposer just optimised against — scoring
                # candidates on their own training questions is precisely how a shard-specific hack wins.
                scored = {}
                for c in pool:
                    rows = [isolated_solve(run_dir, c, d, q.question, args.solve_timeout,
                                           f"pub_b{bi}_{c}_{j}")
                            for j, (d, q) in enumerate(pub_items)]
                    scored[c] = sum(bridge.is_correct(r.get("result") or {}, g)
                                    for r, g in zip(rows, pub_golds))
                sel_on = f"public/{len(pub_items)}"
            else:
                scored = {}
                for c in pool:
                    res_c = cand_results.get(c)
                    if res_c is None:
                        continue
                    scored[c] = sum(bridge.is_correct(r, g) for r, g in zip(res_c, batch_golds[bi]))
                sel_on = f"shard/{len(batch)}"
            if not scored:
                H = final_candidates[0]
            else:
                best = max(scored.values())
                winners = [c for c in pool if scored.get(c) == best]
                # On a TIE keep the NEW candidate. At equal measured accuracy it still carries this round's
                # new mechanisms, which are the raw material the merger aggregates. Falling back to the
                # parent on ties strands every client at the broadcast harness — small shards tie
                # constantly, and that is exactly how the earlier v2 selection lost questions to `bare`.
                # The parent only wins when every candidate is strictly worse.
                H = next((c for c in winners if c != entering_H), entering_H)
            print(
                f"   batch{bi}: SUPERVISED pick H={H} on {sel_on} "
                f"(scores {', '.join(f'{c}={sc}' for c, sc in scored.items())})",
                flush=True,
            )
            # Record, per question, what the chosen harness changed relative to the harness this batch
            # STARTED from. In a federated run that entering harness is the broadcast global, so this file
            # says exactly which questions this client newly solved — the capability an aggregator has to
            # preserve. It is computed here for free; recomputing it centrally would cost a full re-run.
            enter_res, pick_res = cand_results.get(entering_H), cand_results.get(H)
            if enter_res is not None and pick_res is not None:
                verdicts = []
                for j, (db_id, q) in enumerate(batch):
                    was = bool(bridge.is_correct(enter_res[j], batch_golds[bi][j]))
                    now = bool(bridge.is_correct(pick_res[j], batch_golds[bi][j]))
                    verdicts.append({"q": j, "db_id": db_id,
                                     "question": q.question.partition("\nHint:")[0].strip()[:300],
    ...
    print(f"\n######### RESULT ({'supervised train-then-test' if args.supervised else 'test-time / transductive'}) — final harness H = {H} #########", flush=True)
