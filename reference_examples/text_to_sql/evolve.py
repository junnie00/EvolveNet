...
accuracy (a rising curve), with a frozen weak solver and no training?
    ...
    YINLI_API_KEY=<k> YINLI_KEY_EXAMINER=<x> \
    python -m reference_examples.text_to_sql.evolve --db california_schools --shuffle 1 --cap 24 --iterations 5
"""
import argparse
import importlib
import json
...
PKG = "reference_examples.text_to_sql"
PKG_DIR = Path(__file__).parent
AGENTS_DIR = PKG_DIR / "agents"
MH_ROOT = PKG_DIR.parent.parent
...
def task_prompt(iteration, run_dir, n):
    return (
        ...
        f"- harness code lives in `reference_examples/text_to_sql/agents/` (read the best ones; write new ones there)\n"
        f"- `reference_examples/text_to_sql/harness_base.py` — the SQLHarness interface\n\n"
