...
class SolverCache:
    ...
    def get_or_call(self, parts, produce):
        ...
        value = produce()
        with self._lock:
            return self._commit(k, value)

    def _commit(self, k, value):
        """Store `value` for `k` unless another process got there first; return whichever value wins.

        Check-and-set has to happen INSIDE the file lock. Doing the check outside leaves a window where
        two workers both find the key absent, both call the API, and both store a different reply to the
        same prompt — whichever writes last wins, so runs that should be identical differ on a few
        questions. Under the lock, the first writer's value becomes the single answer everyone uses.
        """
        try:
            ...
            with open(self.path + ".lock", "w") as lock:
                ...
                try:
                    try:
                        disk = json.load(open(self.path, encoding="utf-8"))
                    except Exception:  # noqa: BLE001  — absent or corrupt cache is simply an empty one
                        disk = {}
                    if k in disk:                    # someone answered it first — adopt theirs
                        self._data.update(disk)
                        return disk[k]
                    disk.update(self._data)          # merge: never evict another process's entries
                    disk[k] = value
                    tmp = self.path + ".tmp"
