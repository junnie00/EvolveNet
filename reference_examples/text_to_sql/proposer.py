...
# In a FEDERATED run each client sees only ITS OWN slice (often a single database). The default prompt's
# repeated "keep it GENERAL" is correct for centralized training but actively harmful here: it pushes every
# client toward the same generic mechanisms, so their harnesses converge and there is nothing left for the
# server to aggregate (measured: 83-94% pairwise agreement across clients, complementary set only 7 items).
# When specialist mode is on, the client is instead told to become the EXPERT of its own slice — to mine the
# recurring, slice-specific failure modes and schema conventions the generic harness misses. "No hardcoding a
# single question's answer" still holds; what changes is that knowledge specific to THIS slice's schema and
# question style is now the goal, not something to avoid.
SPECIALIST_NOTE = (
    "\n\nSPECIALIST MODE — you serve ONE client whose data is the slice in these traces (often a single "
    "database). Do NOT aim for a generic harness that works everywhere; aim to become the EXPERT of THIS "
    "slice. Study the traces for the recurring, slice-specific failures: which of this schema's similar "
    "tables/columns the coder confuses, which stored-value formats it mis-handles, which multi-step patterns "
    "this question style needs. Encode that specific knowledge — table/column disambiguation, value formats, "
    "join paths particular to THIS schema — as durable mechanisms. (Still no hardcoding one question's "
    "literal answer; schema- and slice-level knowledge is exactly what you SHOULD capture.) The server will "
    "later merge your expertise with other clients' — the more your harness knows that theirs cannot, the "
    "more the federation gains.")
...
def propose_batch(tk):
    ...
    prompt = (
        ...
        f"IF a trace begins with a `GRADED OUTCOME` section, that verdict is GROUND TRUTH for that question — "
        f"it is the single most valuable thing in the trace. Spend your effort on the questions marked "
        f"INCORRECT: for each, work out from the steps and the back-translation WHY the answer was wrong, then "
        f"fix that cause generally. Equally important, do NOT disturb the mechanisms responsible for the "
        f"questions marked CORRECT — a change that fixes one question and breaks two is a regression. Note "
        f"that a question can be marked INCORRECT even though the SQL ran and returned rows; running is not "
        f"the same as answering, and by far the most common failure here is a query that executes cleanly and "
        f"still returns the wrong thing.\n\n"
        ...
        + (SPECIALIST_NOTE if tk.get("specialist") else ""))
    claude_wrapper.run(prompt=prompt, model=tk["model"], allowed_tools=PROPOSER_TOOLS, cwd=str(MH_ROOT),
...
def sample_branches(branches, trace_dir, run_dir, tag, run_name, batch, model, timeout,
                    solve_timeout=180, role_offset=0, specialist=False):
    ...
    ``role_offset`` shifts which search role each branch gets. It exists for the federated setting:
    with G=1 every branch is gi=0 and would otherwise always draw the conservative-repair role, so all
    clients would explore identically and their deltas would collapse into near-duplicates. Offsetting
    by the client index spreads the roles ACROSS clients instead of within one client. It shifts the
    role only — ``branch_id`` in the proposal card stays ``gi``.
    """
    ...
    for gi, (base_candidate, name) in enumerate(zip(branches, names)):
        ...
        task = {
            ...
            "diversity": candidate_diversity(gi + role_offset), "specialist": specialist,
        }
