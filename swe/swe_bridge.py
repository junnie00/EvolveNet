...
def load_instances(ids=None, shuffle_seed=42, limit=None):
    ...
    rows = {
        x["instance_id"]: x
        for x in load_dataset(DATASET, split="test", revision=DATASET_REVISION)
    }
    if ids is not None:
...
class _CachedModel:
    ...
    @staticmethod
    def _key_of(messages):
        """Key on what the model actually SEES, not on the bookkeeping attached to it.

        LitellmModel stamps every reply it returns with `extra = {..., "timestamp": time.time(), "response":
        <full response object>}`, and that reply is appended to the conversation. So from the SECOND call
        onward the message list carries a timestamp, and a naive key over the whole list can never hit —
        measured: two runs of the same instance shared messages 1-3 and diverged at message 4, cutting what
        should have been a full replay down to a 20% speedup."""
        out = []
        for m in messages:
            e = {"role": m.get("role"), "content": m.get("content"),
                 "reasoning_content": m.get("reasoning_content")}
            tcs = m.get("tool_calls") or []
            e["tool_calls"] = [{"name": (tc.get("function") or {}).get("name"),
...
BUDGET_FLAG = os.environ.get("USTC_BUDGET_FLAG", "/tmp/ustc_budget_exhausted")
BUDGET_WAIT = float(os.environ.get("SWE_BUDGET_WAIT", "3600"))


def wait_for_budget(poll=60):
    """Block while the endpoint's budget window is exhausted, instead of running rollouts that cannot work.

    The proxy latches on a budget error and writes BUDGET_FLAG. Without this, every rollout attempted during
    an exhausted window comes back with n_calls=0 and an empty patch — and an empty patch is exactly what a
    genuine model failure looks like. That is not a slow run, it is a CORRUPTED one: in a TTHE batch it made
    all four round-1 candidates come back empty, and the resulting "no effect" conclusion was not a
    measurement at all. Waiting costs wall-clock; not waiting costs the experiment."""
    waited = 0.0
    while os.path.exists(BUDGET_FLAG) and waited < BUDGET_WAIT:
        if waited == 0:
            try:
                why = json.load(open(BUDGET_FLAG)).get("upstream", "")
            except Exception:  # noqa: BLE001
                why = ""
            print(f"   [budget] window exhausted, pausing rather than producing empty rollouts. {why[:200]}",
                  flush=True)
        time.sleep(poll)
        waited += poll
    if waited:
        print(f"   [budget] resumed after {waited / 60:.0f} min", flush=True)
    return not os.path.exists(BUDGET_FLAG)
...
def is_correct_batch(items, run_id):
    ...
    if todo:
        ...
        # The swebench CLI otherwise reloads DATASET from the mutable Hub default.
        # Pass the already pinned instances through its supported local-JSON path.
        dataset_path = LOGS / f"dataset_{run_id}.json"
        dataset_path.write_text(json.dumps([inst for inst, _, _ in todo], indent=2))
        iids = [inst["instance_id"] for inst, _, _ in todo]
        subprocess.run(
            [_sys.executable, "-m", "swebench.harness.run_evaluation",
