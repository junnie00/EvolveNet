...
def main():
    ...
    ap.add_argument("--supervised", action="store_true",
                    help="train-then-test mode: this slice is TRAINING data with usable labels. Traces state "
                         "each patch's graded verdict (official harness, cached by patch hash), and selection "
                         "scores candidates by measured resolved-count instead of a judge's label-free "
                         "investigation. The original transductive protocol must leave this off.")
    ...
    print(f"\n######### harness optimization — SWE-bench Verified — mode={'SUPERVISED train-then-test' if args.supervised else 'transductive test-time'} #########")
    repos = {}
    for inst in items:
    ...
    def write_trace(trace_dir, name, j, instance, patch, steps, verdict=None):
        ...
        if verdict is not None:
            ...
            # legitimately available and is the single most valuable line in the trace.
            L.append(f"## GRADED OUTCOME (ground truth for this training instance)\n"
                     f"**This harness's patch {'RESOLVED' if verdict else 'did NOT resolve'} the issue.**\n"
                     + ("Preserve whatever produced this.\n" if verdict else
                        "This is an instance to fix. Note a patch can fail even though the trajectory looks "
                        "clean — reproducing the issue and passing the repo's own tests is necessary, not "
                        "sufficient; the official hidden suite decides.\n"))
        L += [
    ...
    cand_gold = {}     # harness name -> {instance_id: resolved}; the official harness caches by patch hash
    ...
    def observe(name, batch, trace_dir):
        ...
        return list of patches. Under --supervised, gold runs ONCE per candidate as a BATCH after all patches
        are in (the official harness is expensive; identical patches dedupe via the hash cache), and each
        trace then carries its GRADED OUTCOME header."""
        ...
        def one(ji):
            j, inst = ji
            h = cls(inst)
            patches[j] = safe_solve(h, args.solve_timeout)
            traces[j] = getattr(h, "_trace", [])
        with ThreadPoolExecutor(max_workers=min(len(batch), 5)) as ex:
            list(ex.map(one, list(enumerate(batch))))
        gold = None
        if args.supervised:
            gold = bridge.is_correct_batch(list(zip(batch, patches)), run_id=f"{args.run_name}_{name}_obs")
            cand_gold[name] = gold
        for j, inst in enumerate(batch):
            v = gold.get(inst["instance_id"], False) if gold is not None else None
            write_trace(trace_dir, name, j, inst, patches[j], traces[j], v)
        return patches
    ...
    for bi, batch in enumerate(batches):
        ...
        entering_H = H
        if args.supervised:
            # Selection by MEASURED resolved-count (already computed per candidate at observe time; cached by
            # patch hash). The entering harness is in the pool, so a batch can never hand back something
            # measurably worse than it was given. On a TIE keep the NEW candidate — at equal measured
            # accuracy it still carries this round's new mechanisms, the raw material an aggregator needs.
            scored = {c: sum(cand_gold.get(c, {}).get(i["instance_id"], False) for i in batch) for c in final}
            best = max(scored.values())
            winners = [c for c in final if scored[c] == best]
        ...
        if args.supervised and entering_H in cand_gold and H in cand_gold:
            verdicts = []
            for j, inst in enumerate(batch):
                was = bool(cand_gold[entering_H].get(inst["instance_id"], False))
                now = bool(cand_gold[H].get(inst["instance_id"], False))
                verdicts.append({"q": j, "qid": inst["instance_id"],
                                 "question": inst["problem_statement"][:300],
