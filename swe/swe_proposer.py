...
import claude_wrapper  # root-level copy, decoupled from the running SQL experiments
...
SPECIALIST_NOTE = (
    "\n\nSPECIALIST MODE — you serve ONE client whose data is the slice in these traces (a single "
    "repository or small repo family). Do NOT aim for a generic harness; aim to become the EXPERT of "
    "THIS repo: its test layout, its reproduction workflow, its failure patterns (import cycles, "
    "settings, fixtures), the prompts and verification steps that work HERE. Encode that repo-specific "
    "knowledge as durable mechanisms. (No per-issue hardcoding.) The server later combines your "
    "expertise with other clients'.")
